...
CubicBspline::Delete_Control(const unsigned short posn)
{
    ...
    if (posn >= n)
		throw new GenericException("CubicBspline::Delete_Control - Posn out of range");
...
CubicBspline::Evaluate_Point(const float t, float *pt)
{
    ...
    if (posn > n - 4 && ! loop)
		throw new GenericException("CubicBspline::EvaluatePoint - Parameter value out of range");
    ...
    ** first and last control points (unless it loops.) */
    ...
    for (i = 0, k = 0;i < new_n;i+=2, k++)
    {
		...
		for (j = 0;j < d;j++)
		{
			...
			if (i + 1 < new_n)
				new_c[i+1][j] = 0.125f * (c_pts[p0][j] + 6.0f * c_pts[p1][j]+ c_pts[p2][j]);
		}
    }
